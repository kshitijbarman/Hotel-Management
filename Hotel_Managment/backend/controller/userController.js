...
//   try {
//     const { firstname, lastname, email, phone, password, gender, age } =
//       req.body;
...
//     console.log("email users ka:", email ? "Exists" : "Missing");
...
//     // const logInfo = new userLogModel({
//     //   userId: user._id,
//     //   action: "signup",
//     //   systemName: device,
//     //   ipAddress: clientIp,
...
//     // Temporarily storing user info and OTP
//     otpStore.set(email, {
//       otp,
//       expires: Date.now() + 10 * 60 * 1000,
//       userData: {
...
//     // Optional: Save user log activity
//     // const log = new userLogModel({
//     //   userId: null, // will set after full signup verification
//     //   action: "signup",
//     //   systemName: device,
//     //   ipAddress: clientIp,
//     // });
...
exports.signup = async (req, res) => {
  try {
    ...
    // Temporarily store user data + OTP
    otpStore.set(email, {
      otp,
      expires: Date.now() + 10 * 60 * 1000,
      userData: {
...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    const { firstname, lastname, phone, password, gender, age, role } =
      stored.userData;
    const user = new User({
      firstname,
      lastname,
...
exports.getMe = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select(
      "firstname lastname email phone gender age role profileImage"
    );
...
exports.getUsers = async (req, res) => {
  try {
    ...
    const users = await User.find().select(
      "firstname lastname email isDisabled"
    );
...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find().select(
      "firstname lastname email role isDisabled"
    );
    res.json({ userData: users });
  } catch (error) {
    console.error("getUsers error:", error);
...
exports.updateMe = async (req, res) => {
  try {
    ...
    if (gender && !["male", "female", "other"].includes(gender)) {
      return res
        .status(400)
        .json({ message: "Invalid gender. Must be male, female, or other" });
    }
    if (age && (isNaN(age) || age < 1 || age > 120)) {
      return res
        .status(400)
        .json({ message: "Invalid age. Must be between 1 and 120" });
    }
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select(
      "email firstname lastname backgroundImage isDisabled phone gender age role profileImage"
    );
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
