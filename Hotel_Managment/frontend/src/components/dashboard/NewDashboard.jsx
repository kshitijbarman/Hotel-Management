...
const API_URL = "https://hotel-management-backend-rgpk.onrender.com";
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const calculateRevenue = () => {
      ...
      for (const booking of bookings) {
        ...
        if (!room || !room.hotel || !room.hotel.city || !room.hotel.city.state)
          continue;
      ...
      const byState = Object.entries(byStateMap).map(
        ([stateName, revenue]) => ({ stateName, revenue })
      );
      const byCity = Object.entries(byCityMap).map(([cityName, revenue]) => ({
        cityName,
        revenue,
      }));
      const byHotel = Object.entries(byHotelMap).map(
        ([hotelName, revenue]) => ({ hotelName, revenue })
      );
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (endDate) {
      filtered = filtered.filter(
        (booking) => new Date(booking.checkIn) <= new Date(endDate)
      );
    }
    ...
    if (selectedHotelCity && currentScreen === "hotels") {
      const fetchHotels = async () => {
        try {
          setHotelsLoading(true);
          const response = await api.get(
            `api/cities/${selectedHotelCity}/hotels`
          );
    ...
    if (selectedRoomState && currentScreen === "rooms") {
      const fetchCities = async () => {
        try {
          setRoomsLoading(true);
          const response = await api.get(
            `/api/states/${selectedRoomState}/cities`
          );
    ...
    if (selectedRoomCity && currentScreen === "rooms") {
      const fetchHotels = async () => {
        try {
          setRoomsLoading(true);
          const response = await api.get(
            `/api/cities/${selectedRoomCity}/hotels`
          );
  ...
  const uniqueStates = [
    ...new Set(
      bookings.map((b) => b.roomId?.hotel?.city?.state?.name).filter(Boolean)
    ),
  ];
  const uniqueCities = [
    ...new Set(
      bookings.map((b) => b.roomId?.hotel?.city?.name).filter(Boolean)
    ),
  ];
  const uniqueHotels = [
    ...new Set(bookings.map((b) => b.roomId?.hotel?.name).filter(Boolean)),
  ];

  const uniqueBookingStatuses = [
    ...new Set(bookingsList.map((b) => b.status).filter(Boolean)),
  ];
  const uniqueBookingHotels = [
    ...new Set(bookingsList.map((b) => b.roomId?.hotel?.name).filter(Boolean)),
  ];
                  ...
                  <tbody className="bg-white/50 dark:bg-gray-800/50 divide-y divide-gray-200 dark:divide-gray-700">
                    {profileUsers.length > 0 ? (
                      ...
                      <tr>
                        <td
                          colSpan="3"
                          className="px-6 py-4 text-center text-gray-700 dark:text-gray-200"
                        >
                          No users found.
                        </td>
                      </tr>
                ...
                <div className="bg-white/80 dark:bg-gray-800/80 rounded-2xl shadow-xl overflow-x-auto backdrop-blur-md">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-100/80 dark:bg-gray-700/80">
                      <tr>
                        ...
                        <th
                          className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider cursor-pointer"
                          onClick={() =>
                            setBookingsSort({
                              field: "checkIn",
                              order:
                                bookingsSort.order === "asc" ? "desc" : "asc",
                            })
                          }
                        >
                          Check-in{" "}
                  ...
                  <div className="flex justify-between items-center px-6 py-4">
                    ...
                    <button
                      onClick={() =>
                        setBookingsPage((p) =>
                          Math.min(p + 1, totalBookingPages)
                        )
                      }
              ...
              <>
                ...
                <div className="bg-white/80 dark:bg-gray-800/80 rounded-2xl shadow-xl overflow-x-auto backdrop-blur-md">
                  ...
                  <div className="flex justify-between items-center px-6 py-4">
                    ...
                    <button
                      onClick={() =>
                        setRoomsPage((p) => Math.min(p + 1, totalRoomPages))
                      }
